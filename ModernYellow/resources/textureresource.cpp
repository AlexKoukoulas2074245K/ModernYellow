    ...
    switch (step)
    {
        ...
        case 3:
        {
            for (int y = 0; y < area.h; ++y)
            {
                for (int x = 0; x < area.w; ++x)
                {
                    if (m_pixelSnapshot[y][x] == 0) continue;
    ...
    const uint32 height) const
{
    ...
    SDL_Rect texcoords
	{
		static_cast<int32>(tu),
		static_cast<int32>(tv),
...
void TextureResource::constructPixelSnapshot(const SDL_Rect& area)
{
    ...
    {
        m_pixelSnapshot.push_back(std::vector<uint32>(area.w));
        for (auto x = 0; x < area.w; ++x)
        {            
            m_pixelSnapshot[y][x] = getPixelAt(x + area.x, y + area.y);
        }
    }
